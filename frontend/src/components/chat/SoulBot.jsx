...
import { aurabotService } from '@/services/aurabotService';
...
const THERAPIST_PROMPTS = [
  ...
  'Mindfulness techniques to recommend',
];
...
export function SoulBot() {
  ...
  const sendMessage = async (rawMessage) => {
    ...
    try {
      // Send message to n8n webhook with user context for session memory
      const reply = await aurabotService.sendMessage(
        text,
        userId,
